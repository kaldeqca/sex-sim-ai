/**
 * A definitive, multi-stage parser for JAPANESE (JP) to reliably extract
...
export function parseAdvancedJSON(text, mode) {
    if (!text || typeof text !== 'string') {
        throw new Error("無効な入力：テキストは空でない文字列である必要があります。");
    }
    ...
    const finalResult = {
        mainText: parsedJson.mainText || narrativeStr || "[mainTextが見つかりません]",
...
function validateContent(parsedObject, mode) {
    ...
    if (mode === 'classic') {
        ...
        if (option3 && countChars(option3) < 5) {
            throw new Error("解析失敗：コンテンツが破損しています。クラシックモードの「option3」の文字数が5文字未満です。");
        }
    } else if (mode === 'realistic') {
        const reasoning = parsedObject.coreState?.reasoning;
        if (reasoning && countChars(reasoning) < 5) {
            throw new Error("解析失敗：コンテンツが破損しています。リアリスティックモードの「reasoning」の文字数が5文字未満です。");
        }
    }
}
